...
class DaoGrant(gl.Contract):
    ...
    Workflow:
      ...
      4. Owner calls close_applications() then finalize_scoring()
      5. Owner calls distribute() to send funds to top N recipients

    State flow:
      OPEN -> REVIEW -> SCORED -> DISTRIBUTED
    ...
    app_scores: TreeMap[u256, u256]
    app_verdicts: TreeMap[u256, str]
    app_funded: TreeMap[u256, u256]

    # winner flags stored on-chain as u256 (0=not winner, 1=winner)
    app_is_winner: TreeMap[u256, u256]
    ...
    def get_application(self, app_id: u256) -> str:
        ...
        return (
            ...
            '", "applicant": "' + self.app_applicants.get(
                app_id, Address("0x0000000000000000000000000000000000000000")
            ).as_hex +
            '", "score": ' + str(self.app_scores.get(app_id, u256(0))) +
            ', "funded_wei": ' + str(self.app_funded.get(app_id, u256(0))) +
            ', "is_winner": ' + str(self.app_is_winner.get(app_id, u256(0))) +
    ...
    def submit_application(
        ...
        """
        Submit a grant application. Triggers AI scoring via prompt_comparative.
        All validators fetch project_url and score against rubric.
        Must agree on TIER before score is committed.
        """
        ...
        self.app_is_winner[app_id] = u256(0)
    ...
    def distribute(self) -> None:
        ...
        Uses on-chain TreeMap app_is_winner to mark winners (avoids local TreeMap bug).
        ...
        # Select top N by score — greedy scan, mark winners in on-chain TreeMap
        while distributed_count < actual_winners:
            ...
            while j < self.application_count:
                if self.app_is_winner.get(j, u256(0)) == u256(0):
            ...
            self.app_is_winner[best_id] = u256(1)
        ...
        if distributed_count > u256(0):
            ...
            while k < self.application_count:
                if self.app_is_winner.get(k, u256(0)) == u256(1):
